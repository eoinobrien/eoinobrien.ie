...
<MarkdownContent className='center-content'>
...
<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <Card>
        ...
        <div className="border-t-2 border-stone-300 dark:border-stone-700 mt-2 pt-2 text-right text-sm">
            <Link href="/posts">More projects</Link>
        </div>
    </Card>
    <Card>
        ...
        <CompressedItemsList path="posts" items={
                  props.posts.filter(post => post.types.indexOf(PostType.POST) > -1)
                      .slice(0, 4)
                      .map(post => { return { title: post.title, subtitle: post.subtitle, url: `/posts/${post.slug}`, date: post.date } } )} />
        <div className="border-t-2 border-stone-300 dark:border-stone-700 mt-2 pt-2 text-right text-sm">
