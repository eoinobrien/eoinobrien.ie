...
<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <Card>
        ...
        <CompressedItemsList path="projects" items={props.posts.filter(post => post.types.indexOf(PostType.PROJECT) > -1).slice(0, 4)} />
    </Card>
    <Card>
        <PostTitle size={PostTitleSize.Medium} className="mb-4">Posts</PostTitle>
        <CompressedItemsList path="posts" items={props.posts.filter(post => post.types.indexOf(PostType.POST) > -1).slice(0, 4)} />
